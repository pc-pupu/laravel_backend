...
class CsLicenseController extends Controller
{
     ...
     * GET /api/cs-license/check-draft
     */
    public function checkDraft(Request $request)
    {
        ...
        try {
            ...
            Log::error('CS License Check Draft Error', [
                'error' => $e->getMessage(),
    ...
    /**
     * Get allotment details for CS license
     * GET /api/cs-license/allotment-details
     */
    public function getAllotmentDetails(Request $request)
    {
        ...
        try {
            ...
            if ($existingAppId > 0) {
                ...
                $oldAppId = $this->getMaxCsOnlineApplicationId($uid, ['reject', 'cancel']);
                if ($oldAppId > 0) {
            ...
            Log::error('CS License Get Allotment Details Error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    /**
     * Store or update CS license application
     * POST /api/cs-license/store
     */
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($onlineApplicationId == 0) {
                ...
                $onlineApplicationId = $this->createOnlineApplication($uid, $action, 'CSL', $commonAppData);
    ...
    private function getMaxLicenseApplicationId($uid, $excludeStatuses)
    {
        $result = DB::table('housing_applicant_official_detail as haod')
            ...
            ->where('haod.uid', $uid)
            ->where('hla.type_of_application', 'cs')
    ...
    /**
     * Get max CS online application ID
     */
    private function getMaxCsOnlineApplicationId($uid, $excludeStatuses)
    {
        $result = DB::table('housing_applicant_official_detail as haod')
            ->join('housing_online_application as hoa', 'hoa.applicant_official_detail_id', '=', 'haod.applicant_official_detail_id')
            ->where('haod.uid', $uid)
    ...
    private function createLicenseApplication(Request $request, $onlineApplicationId, $uid)
    {
        ...
        if ($request->hasFile('document')) {
            ...
            $path = $file->store('documents/cs-license', 'public');
