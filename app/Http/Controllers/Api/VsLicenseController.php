...
class VsLicenseController extends Controller
{
     ...
     * GET /api/vs-license/check-draft
     */
    public function checkDraft(Request $request)
    {
        ...
        try {
            ...
            Log::error('VS License Check Draft Error', [
                'error' => $e->getMessage(),
    ...
    /**
     * Get allotment details for VS license
     * GET /api/vs-license/allotment-details
     */
    public function getAllotmentDetails(Request $request)
    {
        ...
        try {
            ...
            if ($existingAppId > 0) {
                ...
                $oldAppId = $this->getMaxVsOnlineApplicationId($uid, ['reject', 'cancel']);
                if ($oldAppId > 0) {
            ...
            Log::error('VS License Get Allotment Details Error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    /**
     * Store or update VS license application
     * POST /api/vs-license/store
     */
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($onlineApplicationId == 0) {
                ...
                $onlineApplicationId = $this->createOnlineApplication($uid, $action, 'VSL', $commonAppData);
    ...
    private function getMaxLicenseApplicationId($uid, $excludeStatuses)
    {
        $result = DB::table('housing_applicant_official_detail as haod')
            ...
            ->where('haod.uid', $uid)
            ->where('hla.type_of_application', 'vs')
    ...
    /**
     * Get max VS online application ID
     */
    private function getMaxVsOnlineApplicationId($uid, $excludeStatuses)
    {
        $result = DB::table('housing_applicant_official_detail as haod')
            ->join('housing_online_application as hoa', 'hoa.applicant_official_detail_id', '=', 'haod.applicant_official_detail_id')
            ->where('haod.uid', $uid)
    ...
    private function createLicenseApplication(Request $request, $onlineApplicationId, $uid)
    {
        ...
        if ($request->hasFile('document')) {
            ...
            $path = $file->store('documents/vs-license', 'public');
    ...
    private function updateLicenseApplication(Request $request, $onlineApplicationId, $uid)
    {
        ...
        if ($request->hasFile('document')) {
            ...
            if ($oldLicense && $oldLicense->document) {
                // Only delete if it's a file path (not a fid)
                if (!is_numeric($oldLicense->document)) {
